...
class WPUSEO {
    ...
    public function wp_title($title, $sep = '|') {
        ...
        if (is_home() || is_front_page() || is_feed() || $this->is_admin_preview_home()) {
            ...
            return apply_filters('wpuseo_title_after_settings', $wpu_title, $bloginfo_name);
        }
    ...
    public function set_metas_ld_json($metas, $metas_json) {
        ...
        if (has_header_image()) {
            ...
            if (is_object($header) && property_exists($header, 'url') && $header->url) {
...
document,\'script\',\'https://connect.facebook.net/en_US/fbevents.js\');';
    ...
    public function prepare_text($source_text, $max_length = 200, $source_type = 'description') {
        ...
        if (isset($words[0]) && strlen($words[0]) > $max_length) {
            $words = array(substr($words[0], 0, $max_length));
            $final_text = $words[0];
        }

        foreach ($words as $word) {
            if ((strlen($final_text) + strlen(' ' . $word)) > $max_length) {
                $final_text = trim($final_text) . $hellip;
                break;
            } else {
                $final_text .= ' ' . $word;
            }
